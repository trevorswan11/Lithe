...
#define READ_SCRIPT_FIELD(FieldType, Type)             \
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		LI_CORE_TRACE("Deserializing scene '{0}'", sceneName);
			...
			for (auto entity : entities)
			{
					...
					if (scriptFields)
					{
						...
						if (entityClass)
						{
							...
							for (auto scriptField : scriptFields)
							{
								...
								LI_CORE_ASSERT(fields.find(name) != fields.end());
				...
				if (spriteRendererComponent)
				{
					...
					else if (spriteRendererComponent["SubTexturePath"])
					{
						...
						std::string subTexturePath = spriteRendererComponent["SubTexturePath"].as<std::string>();
						auto path = Project::GetAssetFileSystemPath(subTexturePath);

						src.Texture = SubTexture2D::CreateFromCoords(
						Texture2D::Create(path.string()), src.Coords, src.CellSize, src.SpriteSize);
					}
